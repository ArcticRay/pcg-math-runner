    ...
    public (float left, float right) AngleConstraints(Direction direction)
{
    ...
    public static void AddBiomeInformation(Mesh mesh, WorldGenerationParameters wgParams, int width, int length, Vector2 offset) {
        Vector2[] biomeInformationUVs = new Vector2[width * length];
        int index = 0;
        Vector3[] vertices = mesh.vertices;
        Vector2[] humidityOctaveOffset = GenerateOctaveOffsets(wgParams.seed + 1, 1, offset);
        Vector2[] temperatureOctaveOffset = GenerateOctaveOffsets(wgParams.seed + 2, 1, offset);
        // Mountain, Plains, Swamp, Pine Forest, Mixed Forest, Leaf Forest
        for (int z = 0; z < width; z++) {
			for (int x = 0; x < length; x++) {
                float humidity = GenerateNoise(x, z, wgParams, humidityOctaveOffset);
                float temperature = GenerateNoise(x, z, wgParams, temperatureOctaveOffset);

                if (humidity < -0.5 && vertices[index].y < 100 && wgParams.fieldBiome) {
                    biomeInformationUVs[index] = new Vector2(((float)Biome.PLAINS), 0);
                }
                else if (temperature < -0.2 || vertices[index].y > 220) {
                    if(wgParams.pineForestBiome) {
                        biomeInformationUVs[index] = new Vector2(((float)Biome.PINE_FOREST), 0);
                    } else if(wgParams.mixedForestBiome) {
                        biomeInformationUVs[index] = new Vector2(((float)Biome.MIXED_FOREST), 0);
                    } else {
                        biomeInformationUVs[index] = new Vector2(((float)Biome.DECIDUOUS_FOREST), 0);
                    }
                }
                else if (temperature > 0.2) {
                    if(wgParams.deciduousForestBiome) {
                        biomeInformationUVs[index] = new Vector2(((float)Biome.DECIDUOUS_FOREST), 0);
                    } else if(wgParams.mixedForestBiome) {
                        biomeInformationUVs[index] = new Vector2(((float)Biome.MIXED_FOREST), 0);
                    } else {
                    ...
                    if(wgParams.mixedForestBiome) {
                        biomeInformationUVs[index] = new Vector2(((float)Biome.MIXED_FOREST), 0);
                    } else if(wgParams.pineForestBiome) {
                        biomeInformationUVs[index] = new Vector2(((float)Biome.PINE_FOREST), 0);
                    } else {
                        biomeInformationUVs[index] = new Vector2(((float)Biome.DECIDUOUS_FOREST), 0);
                    }
